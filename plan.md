...
- `trusted`: `staff` abilities + can edit customer fields + can resend qr link sms
...
4. **Customer page (staff; editing requires trusted role)**
   ...
   - Edit customer profile only if staff role is `trusted`
   ...
   - Utility actions:
     - **Resend customer QR link via SMS** (staff action; sends `/qr?id=<customerId>`; only for `trusted`)
...
- Customer detail + editable fields:
  - Disabled unless role is `trusted`
- Active offers list:
...
- Actions:
  - “Resend QR link via SMS” (Twilio; only for `trusted`)
